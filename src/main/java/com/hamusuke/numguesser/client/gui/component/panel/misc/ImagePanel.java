package com.hamusuke.numguesser.client.gui.component.panel.misc;
...
public class ImagePanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (this.image != null) {
            int x = this.centered ?
                    (int) (this.getX() + this.getWidth() / 2.0D - this.getPreferredSize().getWidth() / 2)
                    : this.getX();
            int y = this.centered ?
                    (int) (this.getY() + this.getHeight() / 2.0D - this.getPreferredSize().getHeight() / 2)
                    : this.getY();
